...
export function Home() {
  ...
  const { data: notifications, error: notifError } = useAsync(
    () => api.getNotifications(),
    []
  )
  // Locally-tracked reads (optimistic) so opening a notification clears its dot.
  ...
  function openNotification(trialId: string | null | undefined, id: string) {
    if (!readIds.includes(id)) {
      setReadIds((prev) => [...prev, id])
      void api.markNotificationRead(id).catch(() => {})
    }
    if (trialId) navigate(`/trials/${trialId}`)
  }

  return (
    <div>
      <Header title={`Hello, ${user?.display_name ?? 'there'}`} />
          ...
          {loading ? (
            ...
            <>
              ...
              {matching.length > 4 && (
                <button
                  onClick={() => navigate('/trials')}
                  className="text-primary mt-3 rounded-sm text-sm font-medium underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-focus)]"
                >
                  See all {matching.length} trials
                </button>
              )}
            </>
          )}
        ...
        <section>
          ...
          {notifError ? (
            ...
            <div className="flex flex-col gap-2">
              {notifications.map((n) => {
                const read = n.read || readIds.includes(n.id)
                return (
                  <Card
                    key={n.id}
                    ...
                    onClick={
                      n.trial_id
                        ? () => openNotification(n.trial_id, n.id)
                        : undefined
                    }
